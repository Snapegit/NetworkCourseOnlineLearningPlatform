...
def yonghu_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if 'yonghumima' in columns:
            password_str='yonghumima'
        else:
            password_str='mima'
...
def yonghu_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if req_dict.get("yonghumima") and "yonghumima" not in yonghu.getallcolumn(yonghu,yonghu) :
            del req_dict["yonghumima"]
        try:
