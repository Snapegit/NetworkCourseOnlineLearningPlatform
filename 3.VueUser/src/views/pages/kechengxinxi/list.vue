			...
			<div class="search_view">
				...
				<div class="search_box">
					<el-select
						class="search_sel"
						clearable
						v-model="searchQuery.erjifenlei" 
						placeholder="二级分类"
						>
						<el-option v-for="item in erjifenleiLists" :label="item" :value="item"></el-option>
					</el-select>
				</div>
			</div>
				...
				<div class="hot_list">
					<div class="hot" v-for="(item,index) in hotList" :key="index" @click="detailClick(item.id)">
						...
						<div class="hot_content">
							<div class="hot_text">{{item.kechengmingcheng}}</div>
						</div>
					</div>
				</div>
			</div>
...
<script setup>
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.yijifenlei = categoryList.value[categoryIndex.value]
		}
